...
createTimeSeries <- function (data, y_lab) {
  ...
  # Most  plot
  p <- ggplot(data, aes(x=Date, y=Value, group = 1)) +
...
list_of_nffd_df = list()


for(m in 1:12) {

  month = sprintf("%02d",m)
  file = paste("../data/ensmax.COM.Tmin", month, ".csv", sep="")
  

  tmin_month <- read.csv(file, sep=',', header = TRUE)
  tmin_month <- tmin_month[tmin_month$scenario == 'historical',][c("Year",model)]
  tmin_month[model] <- lapply(relevant_data[model], function(tm) nffd(m, tm))
  tmin_month["Month"] <- m
  tmin_month["Value"] <- tmin_month[model]

  
  tmin_month$Date<-as.Date(with(tmin_month,paste(Year,Month, "01", sep="-")),"%Y-%m-%d")
  
  list_of_nffd_df[[m]] <-tmin_month 
}

nffd_all <- data.table::rbindlist(list_of_nffd_df)
View(nffd_all)
...
```{r}
createTimeSeries(nffd_all, "NFFD Historical")
```


```{r}
agg = aggregate(nffd_all,by = list(nffd_all$Year),FUN = mean)
createTimeSeries(agg, "NFFD Historical")
